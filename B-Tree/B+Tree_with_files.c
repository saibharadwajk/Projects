...
void insert_q(q_type *,keynode_type *);
keynode_type *delete_q(q_type *);
int isEmpty(q_type *);
void breadth_traversal(database *);
void initialize_q(q_type *);
//                                               Main Function
//**********************************************************************************************************************
int f_dnode=0;
main()
{
    ...
    int key1,key2,back,count,file_opt;
    status_code sc;
    database *dptr;
    ...
    printf("\n\nENTER 1 TO LOAD DATA FROM DISK ELSE 0 TO CONTINUE NORMALLY... ");
    scanf("%d",&file_opt);
    if(file_opt==1)
    {
        fp = fopen("database.txt","r");
        fscanf(fp,"%d",&count);
        printf("the records are %d\n",count);
        i=0;
        while(i<count)
        {
            fscanf(fp,"%d",&record.lib_ACC_num);
            fscanf(fp,"%s",record.b_name);
            /*
            fscanf(fp,"%s",record.a_name);
            fscanf(fp,"%d",&record.ISBN_num);
            fscanf(fp,"%s",record.pub_name);
            fscanf(fp,"%f",&record.b_price);
            */
    ...
    count=getNumRecords(dptr);
    printf("Before saving the number of records are %d",count);
    fprintf(fp,"%d",count);
    if(count!=0)
    {
        kptr=dptr->root;
        ...
        while(ptr!=NULL)
        {
            ...
            {
                fprintf(fp,"\n");
                fprintf(fp,"\t%d",ptr->records[i].lib_ACC_num);
                fprintf(fp,"\t%s",ptr->records[i].b_name);
                /*
                fprintf(fp,"\t%s",record.a_name);
                fprintf(fp,"\t%d",record.ISBN_num);
                fprintf(fp,"\t%s",record.pub_name);
                fprintf(fp,"\t%f",record.b_price);
            */
                i++;
            }
            ptr=ptr->next;
...
status_code insert(database *dptr)
{
    ...
    /*printf("\nENTER AUTHOR NAME : ")
    scanf("%s",record.a_name);
    printf("\nENTER ISBN NUMBER : ");
    scanf("%d",&record.ISBN_num);
    printf("\nENTER PUBLISHER'S NAME : ",record.pub_name);
    scanf("%s",record.pub_name);
    printf("\nENTER BOOK'S PRICE : ");
    scanf("%f",&record.b_price);
    */
    insert_data(dptr,record);
}
    ...
    else
    {
        ...
        if(dcurr==NULL)
        {
            ...
            else
            {
                ...
                dcurr1=make_datanode(); // make a new datanode and transfer all the functions
        ...
        else //dcurr!=NULL
        {
            ...
            else
            {
                if(dcurr->active_rec > MAX_RECS)
                {
                    ...
                    dcurr1->parent_ptr=dcurr->parent_ptr;
                    active_rec = dcurr->active_rec;
                    for(j=0,i=(MAX_RECS+1)/2; i<active_rec; i++,j++) // make an new node transfer the records
                    {
                        dcurr1->records[j]=dcurr->records[i];
                        dcurr1->active_rec+=1;
                    }
                    ...
                    if(f_dnode!=0) // if it is not first data node that is being split
                    {
                        insert_key(curr,current);
                        if(curr->active_keys<=MAX_KEYS) // if parent is ok after inserting key
                        {
                        ...
                        else // if parent has excess keys after inserting
                        {
                            ptr=make_keynode(1);
                    ...
                    else // if it is the first datanode that is being split
                    {
...
//****************************************************************************************************************************
void breadth_traversal(database *dptr)
{
    keynode_type *ptr;
    q_type *q,que;
    q=&que;
    ...
    {
        insert_q(q,dptr->root);
        while(!isEmpty(q)) // queue is not empty
        {
            ptr=delete_q(q); // delete from queue
            i=0;
            while(i<ptr->active_keys) // print all keys
            {
                printf("\t %d",ptr->key[i]);
                i++;
            }
            if(ptr->u_type!=1)
            {
                for(i=0;i<=ptr->active_keys;i++) // add its children
                {
                    insert_q(q,ptr->ptr_type.ptr[i]);
                }
            }
            getch();
            printf("\n");
...
//                                                      insert_q Function
//******************************************************************************************************************************
void insert_q(q_type *q_ptr,keynode_type *kptr)
{
    kptr->next=NULL;
    if(q_ptr->front==NULL)
    {
    ...
    {
        q_ptr->rear->next=kptr;
        q_ptr->rear=kptr;
    }
}
//                                                        delete_q Function
//*****************************************************************************************************************************
keynode_type * delete_q(q_type *q_ptr)
{
    keynode_type *ptr;
    ptr=q_ptr->front;
    q_ptr->front=ptr->next;
    if(q_ptr->front==NULL)
    {
        q_ptr->rear =NULL;
    }
